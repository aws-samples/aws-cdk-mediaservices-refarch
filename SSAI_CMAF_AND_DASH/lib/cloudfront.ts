...
export class CloudFront extends Construct {
   ...
   public readonly emtCmafPlayback: string;
   public readonly emtDashPlayback: string;
   public readonly emtHlsSession: string;
   public readonly emtCmafSession: string;
   ...
   private readonly DESCRIPTIONDISTRIBUTIONDEMO = " - Demo website for SSAI";
  ...
  constructor(scope: Construct, id: string,
    ...
    cmafEndpoint: string,
    dashEndpoint: string,
    hlsConfigNameEMT: string,
    hlsSessionEndpointEMT: string,
    dashConfigNameEMT: string,
    dashSessionEndpointEMT: string,
    props: Secrets){
    super(scope, id);
    ...
    */
    ...
    const mediaTailorHlsHostname = Fn.select(2, Fn.split("/", hlsSessionEndpointEMT));
    const mediaTailorDashHostname = Fn.select(2, Fn.split("/", dashSessionEndpointEMT));
    ...
    const myOriginRequestPolicyEMT = new cloudfront.OriginRequestPolicy(this, 'OriginRequestPolicyEMT', {
      ...
      // Passing all headers through to origin to enable gzipped manifests to be delivered
      // It is not possible to configure the passthrough of the Accept-Encoding header in CloudFront
      // when caching on the behaviour is disabled.
    ...
    const mediaTailorHlsOrigin = new origins.HttpOrigin(
      mediaTailorHlsHostname,
      {
          customHeaders: { 
    ...
    const mediaTailorDashOrigin = new origins.HttpOrigin(
      mediaTailorDashHostname,
      {
          customHeaders: { 
          //👇 Header to provide for accessing MediaPackage origin
          'X-MediaPackage-CDNIdentifier': props.cdnSecret.secretValueFromJson(this.CDNHEADER).unsafeUnwrap().toString(),
    ...
    */
    ...
    const distribution = new cloudfront.Distribution(this, "Distribution", {
      ...
      // Add Behaviours for HLS requests being directed to MediaTailor which should not be cached
      let MediaTailorHlsCacheDisabledPathPatterns = [
        "/v1/session/*/" + hlsConfigNameEMT + "/*",
        "/v1/tracking/*/" + hlsConfigNameEMT + "/*",
        "/v1/m*.m3u8",
      ]
      for ( var pathPattern of MediaTailorHlsCacheDisabledPathPatterns ) {
        distribution.addBehavior(pathPattern, mediaTailorHlsOrigin, {
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
      ...
      // Add Behaviours for HLS requests being directed to MediaTailor which should be cached
      let MediaTailorHlsCacheOptimizedPathPatterns = [
        "/v1/w*",
        "/v1/segment/*"
      ]
      for ( var pathPattern of MediaTailorHlsCacheOptimizedPathPatterns ) {
        distribution.addBehavior(pathPattern, mediaTailorHlsOrigin, {
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
          allowedMethods: cloudfront.AllowedMethods.ALLOW_ALL,
          cachePolicy: cloudfront.CachePolicy.CACHING_OPTIMIZED,
      ...
      // Add Behaviours for DASH requests being directed to MediaTailor
      let MediaTailorDashPathPatterns = [
        "/v1/session/*/" + dashConfigNameEMT + "/*",
        "/v1/tracking/*/" + dashConfigNameEMT + "/*",
        "/v1/dash/*.mpd"
      ]
      for ( var pathPattern of MediaTailorDashPathPatterns ) {
        distribution.addBehavior(pathPattern, mediaTailorDashOrigin, {
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
          allowedMethods: cloudfront.AllowedMethods.ALLOW_ALL,
          cachePolicy: cloudfront.CachePolicy.CACHING_DISABLED,
          originRequestPolicy : myOriginRequestPolicyEMT,
      ...
      // Add behaviour for DASH ad beaconing redirects
      // This behaviour is required as many ADS only expect a tracking beacon to be
      // fired once. If these requests are not cached additional tracking requests
      // will be generated when a user scrubs/rewinds to an ad they have previously viewed
      distribution.addBehavior("/v1/dashsegment/*", mediaTailorDashOrigin, {
        viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
        allowedMethods: cloudfront.AllowedMethods.ALLOW_ALL,
        cachePolicy: cloudfront.CachePolicy.CACHING_OPTIMIZED,
        originRequestPolicy : myOriginRequestPolicyEMT,
      ...
      // Add Behaviours for requests for media segments being directed to MediaPackage 
      distribution.addBehavior("/out/v1/*", mediaPackageOrigin, {
    ...
    */
    ...
    const hlsPathEMT = Fn.select(1, Fn.split("/v1/session/", hlsSessionEndpointEMT));
    const hlsSessionPathEMT = Fn.select(1, Fn.split("/v1/", hlsSessionEndpointEMT));
    const dashPathEMT = Fn.select(1, Fn.split("/v1/session/", dashSessionEndpointEMT));
    const dashSessionPathEMT = Fn.select(1, Fn.split("/v1/", dashSessionEndpointEMT));

    this.emtHlsPlayback="https://" + distribution.domainName + '/v1/master/' + hlsPathEMT + 'out/' + hlsPathEMP
    this.emtCmafPlayback="https://" + distribution.domainName + '/v1/master/' + hlsPathEMT + 'out/' + cmafPathEMP 
    this.emtDashPlayback="https://" + distribution.domainName + '/v1/dash/' + dashPathEMT + 'out/' + dashPathEMP
    this.emtHlsSession="https://" + distribution.domainName + '/v1/' + hlsSessionPathEMT + 'out/' + hlsPathEMP
    this.emtCmafSession="https://" + distribution.domainName + '/v1/' + hlsSessionPathEMT + 'out/' + cmafPathEMP
    this.emtDashSession="https://" + distribution.domainName + '/v1/' + dashSessionPathEMT + 'out/' + dashPathEMP
   
    new CfnOutput(this, "MyCloudFrontHlsEndpoint", {
      value: this.emtHlsPlayback,
      exportName: Aws.STACK_NAME + "cloudFrontHlsEndpoint",
      description: "The HLS playback endpoint (origination disabled)",
    });
    new CfnOutput(this, "MyCloudFrontCmafEndpoint", {
      value: this.emtCmafPlayback,
    ...
    new CfnOutput(this, "MyCloudFrontDemoWebSiteEndpoint", {
      ...
      description: "The demo web player to play the CMAF and MPEG DASH",
    });
