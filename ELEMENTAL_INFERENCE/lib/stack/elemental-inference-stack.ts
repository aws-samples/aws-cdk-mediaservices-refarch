...
export class ElementalInferenceStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const mediaLivePolicy = new iam.PolicyDocument({
      statements: [
        new iam.PolicyStatement({
          resources: ["*"],
          actions: [
            // Elemental Inference — control plane FAS (no hyphen)
            "elementalinference:AssociateFeed",
            "elementalinference:DisassociateFeed",
            "elementalinference:GetFeed",
            // Elemental Inference — data plane (hyphenated service namespace)
    ...
    // Attach channel policies (MediaLive PutObject) and endpoint policies (public read)
    mediaPackage.attachPolicies(mediaLiveRole.roleArn);
