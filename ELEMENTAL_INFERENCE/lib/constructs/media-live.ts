...
export class InferenceMediaLive extends Construct {
  ...
  constructor(scope: Construct, id: string, props: InferenceMediaLiveProps) {
    ...
    // Horizontal output group — CMAF requires separate outputs for video and audio
    const horizontalOutputGroup: medialive.CfnChannel.OutputGroupProperty = {
      ...
      outputs: [
        // Video-only outputs
        ...props.videoConfig.horizontal.map((r) => ({
          outputName: r.name,
          videoDescriptionName: r.name,
          audioDescriptionNames: [] as string[],
          captionDescriptionNames: [] as string[],
          outputSettings: {
    ...
    // Vertical output group — CMAF requires separate outputs for video and audio
    const verticalOutputGroup: medialive.CfnChannel.OutputGroupProperty = {
      ...
      outputs: [
        // Video-only outputs
        ...props.videoConfig.vertical.map((r) => ({
          outputName: r.name,
          videoDescriptionName: r.name,
          audioDescriptionNames: [] as string[],
          captionDescriptionNames: [] as string[],
    ...
    const destinations: medialive.CfnChannel.OutputDestinationProperty[] = [
      {
        ...
        mediaPackageSettings: [
          {
            channelGroup: props.horizontalChannel.channelGroupName,
            channelName: props.horizontalChannel.channelName,
          },
        ],
      },
      {
        ...
        mediaPackageSettings: [
          {
            channelGroup: props.verticalChannel.channelGroupName,
            channelName: props.verticalChannel.channelName,
          },
        ],
      },
    ];
