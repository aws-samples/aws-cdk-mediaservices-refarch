  ...
  describe("loadConfig", () => {
    ...
    test("should load configuration successfully", () => {
      ...
      const expectedConfig: MockConfig = {
        event: {
          mediaLive: {
            encodingProfileLocation: path.resolve(
              "/mock/cwd",
              "node_modules/.bin",
              "../relative/path/to/profile.json",
            ),
          },
        },
      };
    ...
    test("should throw ConfigurationError when config key not found", () => {
      ...
      (loadConfig as jest.Mock).mockImplementation(() => {
        throw new ConfigurationError(
          'Configuration key "nonExistentKey" not found in /path/to/config',
        );
      });
    ...
    test("should throw ConfigurationError when require fails", () => {
      ...
      (loadConfig as jest.Mock).mockImplementation(() => {
        throw new ConfigurationError(
          "Failed to load configuration file (/non-existent/path): Module not found",
        );
      });
    ...
    test("should convert relative encoding profile path to absolute path", () => {
      ...
      // Assert
      expect(result.event.mediaLive.encodingProfileLocation).toEqual(
        expectedPath,
      );
    });
