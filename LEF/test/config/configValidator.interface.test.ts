...
import {
  ...
  ConfigurationError,
  ConfigType,
} from "../../lib/config/configValidator";
...
class TestConfigValidator implements IConfigValidator<TestConfig> {
  validateConfig(config: TestConfig): void {
    ...
    if (
      config.event.optional !== undefined &&
      typeof config.event.optional !== "string"
    ) {
      throw new ConfigurationError(
        "Config optional property must be a string if provided",
      );
    }
  }
}
...
describe("IConfigValidator", () => {
  ...
  test("should throw when config is null", () => {
    // Act & Assert
    expect(() =>
      validator.validateConfig(null as unknown as TestConfig),
    ).toThrow(ConfigurationError);
    expect(() =>
      validator.validateConfig(null as unknown as TestConfig),
    ).toThrow("Config cannot be null or undefined");
  });

  test("should throw when config is undefined", () => {
    // Act & Assert
    expect(() =>
      validator.validateConfig(undefined as unknown as TestConfig),
    ).toThrow(ConfigurationError);
    expect(() =>
      validator.validateConfig(undefined as unknown as TestConfig),
    ).toThrow("Config cannot be null or undefined");
  ...
  test("should throw when event is missing", () => {
    ...
    expect(() => validator.validateConfig(invalidConfig)).toThrow(
      "Config must have an event property",
    );
  });
  ...
  test("should throw when name is missing", () => {
    ...
    expect(() => validator.validateConfig(invalidConfig)).toThrow(
      "Config must have a name property",
    );
  });
  ...
  test("should throw when value is missing", () => {
    ...
    expect(() => validator.validateConfig(invalidConfig)).toThrow(
      "Config must have a value property",
    );
  });
